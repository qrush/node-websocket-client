// An implementation of HTML5 Web Sockets
...
var WebSocket = function(url, proto) {
    ...
    //
    // Note that this value is only guaranteed to be defined while
    // readyState is OPEN or CLOSING.
    ...
    var frameFuncs = [
        ...
        function(buf, off) {
            ...
            // For high-byte frames, read the number of bytes that the frame
            // contains
            for (var i = off + 1, length = 0; i < buf.length; i++) {
                length = length * 128 + (buf[i] & 0x7f);

                // We found the end of the frame. Take action.
                if ((buf[i] & 0x80) == 0) {
                    if (length == 0) {
                        // If we've responding to a client-initiated
                        ...
                        throw new Error(
                            'High-byte frames not yet supported'
                        );
                    }
                }
            }

            // We didn't get to the end of the frame header. Wait for some
            // more bytes to show up and try again.
            return 0;
        },
        ...
        // FRAME_CL
        //
        // For this frame type, we don't reset frameType to ensure that we
        // correctly ignore all bytes arriving after this (which will then
        // go through this parser).
        function(buf, off) {
            var nbytes = buf.length - off;

            debug('Dropping ' + nbytes + ' bytes that arrived after ' +
    ...
    // Handle the connection being completely closed
    //
    // This fires off the onclose() and 'close' events to the user of the
    // WebSocket instance.
    ...
    //
    // Writes the 0xff,0x00 bytes and waits for them to be written out on the
    // socket before closing it.
    var closeHandshake = function() {
        var f = function() {
            // If we're responding to a server-initiated shutdown, close the
            // socket
            if (frameType == FRAME_CL) {
                stream.end();
            }
        };

        if (stream.write('\xff\x00', 'binary')) {
            f();
        } else {
            stream.addListner('drain', f);
        }
    };
    ...
    (function() {
        ...
        httpClient.addListener('upgrade', (function() {
            ...
            return function(req, s, head) {
                ...
                //
                // Our 'data' listener is special in that it only does
                // handshaking. Once handshaking is complete, it will replace
                // itself with dataListener.
                ...
                // Fire off the received data at the listener
